    ...
    >
      ...
      <div className="mb-8">
        <button 
          ...
          className="bg-teal-500 text-white px-4 py-2 rounded hover:bg-teal-600 transition duration-200 mb-4"
        >
          {showEditAds ? 'Hide Edit Ads' : 'Edit Ads'}
        </button>
        {showEditAds && (
          <div className={`p-6 rounded-lg shadow-md ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            <h2 className="text-2xl font-bold mb-4">Edit Advertisements</h2>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {ads.map(ad => (
                <div key={ad.id} className={`p-4 rounded-lg shadow-md ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
                  {ad.image && (
                    <img 
                      src={ad.image} 
                      alt="Advertisement" 
                  ...
                  <a 
                    ...
                    className="text-blue-500 hover:underline block overflow-hidden whitespace-nowrap text-overflow-ellipsis"
                  >
                    {ad.link}
                  </a>
                  <div className="mt-2">
                    <button onClick={() => deleteAd(ad.id)} className="bg-red-500 text-white px-2 py-1 rounded">Delete</button>
                  </div>
                </div>
              ))}
            </div>
